...
describe('Sleep', () => {
  ...
  it('should return a users average amount of sleep', () => {
    expect(sleep.returnAverageSleepInfo(1, 'hoursSlept')).to.equal(8.3);
  });

  it('should return a users average quality of sleep', () => {
    expect(sleep.returnAverageSleepInfo(1, 'sleepQuality')).to.equal(2.7);
  });

  it('should return how much a user slept on a particular day', () => {
    expect(sleep.returnSleepInfo(1, '2019/06/15', 'hoursSlept')).to.equal(6.1);
  });

  it('should return the quality of sleep for a particular day', () => {
    expect(sleep.returnSleepInfo(1, '2019/06/16', 'sleepQuality')).to.equal(3.8);
  })

  it('should show how much a user slept per day for a given week', () => {
    expect(sleep.returnSleepInfoByWeek(3, '2019/06/21', 'hoursSlept')).to.eql([10.8, 10.7, 5.3, 9.8, 7.2, 9.4, 8.9]);
  });

  it('should show a users quality of sleep per day for a given week', () => {
    expect(sleep.returnSleepInfoByWeek(3, '2019/06/22', 'sleepQuality').length).to.equal(7);
  });
